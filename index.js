...
const leaveroom = (id) => {
  ...
  lobbies.forEach((c) => {
    ...
    if (c[room].length === 1 && userexist) {
      ...
      lobbies = updatedarr;
      const updatedboss = boss.filter((u) => u[room] === undefined);
      boss = updatedboss;
      const updatedpass = passwords.filter((u) => u[room] === undefined);
  ...
  console.log(passwords, "pass", boss, "boss");
  return joinedRoom;
};
...
io.on("connection", async (socket) => {
  ...
  socket.on("leaveroom", () => {
    ...
    if (room) {
      ...
      io.emit("roominfo", lobbies);
      console.log(
        "A disconnection has been made and they have been removed",
        socket.id
      );
    } else {
      console.log(
        "A disconnection has been made but they werent joined in any room",
        socket.id
      );
    }
  });
